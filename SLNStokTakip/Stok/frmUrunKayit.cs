    ...
    public partial class frmUrunKayit : Form
    {
        ...
        public void Ac(int id)
        {
            try
            {
                ...
                lblGenelNo.Text = ust.GenelNo.ToString(); //Genel No int olduğu için tostring yazdık..
                txtAciklama.Text = ust.Aciklama;
                txtCariAdi.Text = ust.bgFirma.Fadi;
                txtGirisKodu.Text = ust.GirisKod.ToString().PadLeft(7, '0');//Genel No ile aynı..
        ...
        {
            if (e.ColumnIndex == 0)//indexi sıfır olan koda dokunduğumuzda çalışsın.
            {
                    ...
                    if (cell.Value != null)
                    {
                        try
                        {
                            var lst = (from s in _db.bgUrunGiris
                                       ...
                                       select s).First();
                            string ack = lst.UrunAciklama;//ack açıklamanın kısaltması
